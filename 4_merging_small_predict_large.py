...
mkt_median = pd.read_stata(params.mkt_median_path)
mkt_median = mkt_median.set_index('datadate')

# calculate factors
for yr in range(2013, params.year_start-1, -1): 
    print('*'*10,yr, '*'*10) # 2015
    ...
    # slicing 
    network_start = pd.Timestamp(yr - params.network_lag + 1, 1, 1)
    network_end = pd.Timestamp(yr, 12, 31)
    ...
    factor_start = pd.Timestamp(yr + 1, 7, 31)
    factor_end = pd.Timestamp(yr + 2, 6, 30)
    stock_ret_slice = stock_ret[(stock_ret.datadate >= factor_start) & (stock_ret.datadate <= factor_end)]  # 66817
    ...
    if params.fast:
        ...
        try:
           network_yr_panal =  pyreadstat.read_dta(str(params.network_lag) + 'yr_network.dta')[0]
           network_yr_panal =  network_yr_panal[network_yr_panal.year == yr]
           network_yr_panal = network_yr_panal [['cpni', 'cpnj', 'numfriend']]
           network_yr_panal.columns = ['company_i', 'company_j', 'count']
           network_yr_panal.company_i = network_yr_panal.company_i.map(lambda x: str(int(x)))
           network_yr_panal.company_j = network_yr_panal.company_j.map(lambda x: str(int(x)))
        except:
    ...
    for mth in pd.date_range(factor_start, factor_end, freq='M'):
        print('* ', mth.strftime('%Y-%m-%d'))
        ret_slice = stock_ret_slice[stock_ret_slice.datadate == mth] 
        if len(ret_slice)==0:
            continue
        ...
        ret_slice['mcap'] = ret_slice['prccm'] * ret_slice['cshoq'] 
        ret_slice = ret_slice[ret_slice['mcap'].notna()].reset_index(drop=True)
        med = mkt_median.loc[mth].values[0]
        ret_slice.loc[ret_slice['mcap'] >= med, 'trt1m'] = np.nan
        ...
        ret_slice = ret_slice.loc[ret_slice['mcap'] >= med]
        print('Coverage: ', round((len(ret_slice['DN_factor'].dropna())/ len(ret_slice)), 4))
        ret_slice.to_excel(params.data_path + mth.strftime('%Y%m%d') + '.xlsx', index=None)
        del ret_slice
    if params.fast:
        del network_yr 
    else:
